...
"""
Utilities for pre-processing and iterating over the CoNLL 2003 data.
"""
    ...
    for file_path in [train_path, dev_path, test_path]:
        with open(file_path, 'r') as file:
            for line in file:
                ...
                word = DIGIT_RE.sub("0", tokens[1]) if normalize_digits \
                    else tokens[1]
                ner = tokens[4]

                if glove_dict is not None and (word in glove_dict or
                                               word.lower() in glove_dict):
...
def read_data(source_path, word_vocab, char_vocab, ner_vocab,
...
def iterate_batch(data, batch_size, shuffle=False):
    ...
    for start_idx in range(0, len(data), batch_size):
        ...
        cid_inputs = np.empty([len(batch), batch_length, MAX_CHAR_LENGTH],
                              dtype=np.int64)
...
def load_glove(filename, emb_dim, normalize_digits=True):
    ...
    with open(filename) as fin:
        for line in fin:
            ...
            word = tx.utils.compat_as_text(word)
    ...
    def getNext(self, normalize_digits=True):
        ...
        for tokens in lines:
            ...
            word = DIGIT_RE.sub("0", tokens[1]) \
                if normalize_digits else tokens[1]
            ner = tokens[4]
