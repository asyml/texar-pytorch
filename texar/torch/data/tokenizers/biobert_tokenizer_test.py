"""
Unit tests for pre-trained BioBERT tokenizer.
"""
...
from texar.torch.data.tokenizers.biobert_tokenizer import \
    BioBERTTokenizer
from texar.torch.utils.test import pretrained_test


class BioBERTTokenizerTest(unittest.TestCase):
    ...
    def test_model_loading(self):
        for pretrained_model_name in BioBERTTokenizer.available_checkpoints():
            tokenizer = BioBERTTokenizer(
                pretrained_model_name=pretrained_model_name)
            _ = tokenizer.map_text_to_token(u"UNwant\u00E9d,running")

    def test_tokenize(self):
        tokenizer = BioBERTTokenizer.load(self.vocab_file)
    ...
    def test_pretrained_model_list(self):
        model_list_1 = list(BioBERTTokenizer._MODEL2URL.keys())
        model_list_2 = list(BioBERTTokenizer._MAX_INPUT_SIZE.keys())
