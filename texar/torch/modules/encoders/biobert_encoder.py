...
from texar.torch.modules.pretrained.biobert import PretrainedBioBERTMixin
...
class BioBERTEncoder(PretrainedBioBERTMixin, BERTEncoder):
    r"""BioBERT Transformer for encoding sequences.

    This module basically stacks
    :class:`~texar.torch.modules.WordEmbedder`,
    :class:`~texar.torch.modules.PositionEmbedder`,
    :class:`~texar.torch.modules.TransformerEncoder` and a dense pooler.

    Please see :class:`~texar.torch.modules.PretrainedBioBERTMixin` for a
    brief description of BioBERT.

    Args:
        pretrained_model_name (optional): a `str`, the name
            of pre-trained model (e.g., ``biobert-v1.0-pmc``). Please refer
            to :class:`~texar.torch.modules.PretrainedBioBERTMixin` for
        ...
        * If the above two are `None`, the encoder arch is defined by the
        ...
        .. code-block:: python

            {
                ...
                "hidden_size": 768,
                "initializer": None,
                "name": "biobert_encoder",
            }

        Here:

        The default parameters are values for pre-trained BioBERT model.

        `"pretrained_model_name"`: str or None
            The name of the pre-trained BioBERT model. If None, the model
        ...
        `"vocab_size"`: int
            The vocabulary size of `inputs` in BioBERT model.

        `"segment_embed"`: dict
            Hyperparameters for segment embedding layer.

        `"type_vocab_size"`: int
            The vocabulary size of the `segment_ids` passed into BioBert model.
