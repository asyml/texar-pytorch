...
from texar.torch.modules.pretrained.spanbert import PretrainedSpanBERTMixin
...
class SpanBERTEncoder(PretrainedSpanBERTMixin, BERTEncoder):
    r"""SpanBERT Transformer for encoding sequences. Please see
    :class:`~texar.torch.modules.PretrainedSpanBERTMixin` for a brief
    description of SpanBERT.
    ...
    Args:
        pretrained_model_name (optional): a `str`, the name
            of pre-trained model (e.g., ``spanbert-base-cased``). Please refer
            to :class:`~texar.torch.modules.PretrainedSpanBERTMixin` for
        ...
        * If the above two are `None`, the encoder arch is defined by the
        ...
        .. code-block:: python

            {
                ...
                "hidden_size": 768,
                "initializer": None,
                "name": "spanbert_encoder",
            }

        Here:

        The default parameters are values for SpanBERT-Base model.

        `"pretrained_model_name"`: str or None
            The name of the pre-trained SpanBERT model. If None, the model
        ...
        `"vocab_size"`: int
            The vocabulary size of `inputs` in SpanBERT model.
    ...
    def forward(self,  # type: ignore
        ...
        r"""Encodes the inputs. Differing from the standard BERT, the SpanBERT
        model does not use segmentation embedding. As a result, SpanBERT does
        not require `segment_ids` as an input.
        ...
        if segment_ids is not None:
            raise ValueError("segment_ids should be None in SpanBERTEncoder.")
