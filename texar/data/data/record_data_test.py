...
class RecordDataTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cat_in_snow = tx.data.data_utils.maybe_download(
            'https://storage.googleapis.com/download.tensorflow.org/'
            'example_images/320px-Felis_catus-cat_on_snow.jpg',
            self._test_dir, 'cat_0.jpg')
        williamsburg_bridge = tx.data.data_utils.maybe_download(
            'https://storage.googleapis.com/download.tensorflow.org/'
        ...
        with tx.data.RecordData.writer(_tfrecord_filepath,
    ...
    def _run_and_test(self, hparams):
        # Construct database
        record_data = tx.data.RecordData(hparams)
        iterator = tx.data.DataIterator(record_data)
