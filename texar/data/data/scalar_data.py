...
class ScalarData(DataBase):
    ...
    @property
    def collate_fn(self) -> Callable[[List[str]], Batch]:
        ...
        def collate_fn(batch: List[str]) -> Batch:
            dtype = ScalarData._get_dtype(self._hparams.dataset["data_type"])
            batch = np.array(batch, dtype=dtype)
            return Batch(len(batch), label=batch)
    ...
    def list_items(self):
        ...
        Returns:
            A list of strings.
        """
        return [self._hparams.dataset["data_name"]]
    ...
    def data_name(self):
        ...
        :attr:`hparams`.
        """
        return self._hparams.dataset.get("data_name", "data")
